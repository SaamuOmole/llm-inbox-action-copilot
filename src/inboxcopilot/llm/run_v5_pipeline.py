...
from inboxcopilot.rag.context import build_rag_context
...
OUT_PATH = Path(os.getenv("PRED_PATH", "data/predictions/intent_action_v5_rag.jsonl"))
...
def main():
    ...
    with OUT_PATH.open("w", encoding="utf-8") as fout:
        for email_id in gold_idx.keys():
            ...
            try:
                ...
                # Decide whether to use RAG context
                use_rag = (conf in ("low", "medium")) or (len(body) < 400)

                context = ""
                if use_rag:
                    context = build_rag_context(email_id=email_id, subject=subject, body=body)
                    rag_used += 1
    ...
    print(f"OK: {n_ok} | ERR: {n_err} | RAG used: {rag_used}/{n_ok+n_err}")
